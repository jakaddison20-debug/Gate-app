...
function segmentCrossesGate(prevPos,currPos,gate,radius){if(!prevPos)return haversine(currPos,gate)<=radius;const a=metersOffset(gate,prevPos);const b=metersOffset(gate,currPos);const dx=b.x-a.x,dy=b.y-a.y;const lenSq=dx*dx+dy*dy;let t=lenSq===0?0:-(a.x*dx+a.y*dy)/lenSq;t=Math.max(0,Math.min(1,t));const cx=a.x+t*dx,cy=a.y+t*dy;return Math.sqrt(cx*cx+cy*cy)<=radius;}
...
export default function App(){
  ...
  useEffect(()=>{if(!user)return;supabase.from('profiles').select('display_name,avatar_url,bike_name,rider_weight,tire_dry_front,tire_dry_rear,tire_wet_front,tire_wet_rear,shock_mode,shock_psi,shock_spring_rate,shock_lsc,shock_hsc,shock_lsr,shock_hsr,shock_hsb,shock_tokens,shock_sag,fork_mode,fork_psi,fork_spring_rate,fork_lsc,fork_hsc,fork_lsr,fork_hsr,fork_hsb,fork_tokens,fork_sag,bike_notes,fork_notes,shock_notes').eq('id',user.id).single().then(({data})=>{if(data)setSettings(prev=>({...prev,displayName:data.display_name||prev.displayName,avatarUrl:data.avatar_url||null,bikeName:data.bike_name||'',riderWeight:data.rider_weight||'0',tireDryFront:data.tire_dry_front||'0',tireDryRear:data.tire_dry_rear||'0',tireWetFront:data.tire_wet_front||'0',tireWetRear:data.tire_wet_rear||'0',shockMode:data.shock_mode||'psi',shockPsi:data.shock_psi||'0',shockSpringRate:data.shock_spring_rate||'0',shockLsc:data.shock_lsc||'0',shockHsc:data.shock_hsc||'0',shockLsr:data.shock_lsr||'0',shockHsr:data.shock_hsr||'0',shockHsb:data.shock_hsb||'0',shockTokens:data.shock_tokens||'0',shockSag:data.shock_sag||'0',forkMode:data.fork_mode||'psi',forkPsi:data.fork_psi||'0',forkSpringRate:data.fork_spring_rate||'0',forkLsc:data.fork_lsc||'0',forkHsc:data.fork_hsc||'0',forkLsr:data.fork_lsr||'0',forkHsr:data.fork_hsr||'0',forkHsb:data.fork_hsb||'0',forkTokens:data.fork_tokens||'0',forkSag:data.fork_sag||'0',bikeNotes:data.bike_notes||'',forkNotes:data.fork_notes||'',shockNotes:data.shock_notes||''}));});},[user]);
