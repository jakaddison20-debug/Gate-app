...
// ── Stage Detail Sheet ────────────────────────────────────────────────────────
function StageDetailSheet({stage,onClose}){
  ...
  return(
    <button className="tap" onClick={()=>onPress&&onPress(stage)} style={{width:"100%",display:"flex",alignItems:"center",gap:12,padding:"13px 16px",borderBottom:`1px solid ${C.border}`,background:"white",textAlign:"left"}}>
      ...
      <div style={{textAlign:"right",flexShrink:0,display:"flex",alignItems:"center",gap:8}}>
        ...
        <Icon.ChevronRight size={14} color={C.mutedL}/>
      </div>
    </button>
  );
}
...
function RaceScreen({course,stages,onFinish}){
...
useEffect(()=>{if(!navigator.geolocation)return;let centered=false;const id=navigator.geolocation.watchPosition(pos=>{const loc={lat:pos.coords.latitude,lng:pos.coords.longitude};setUserPos(loc);if(!centered){setMapCenter(loc);centered=true;}},err=>console.log(err),{enableHighAccuracy:true,maximumAge:2000,timeout:10000});return()=>navigator.geolocation.clearWatch(id);},[]);
  ...
  return(
    <div ref={containerRef} style={{width:"100%",height:"100vh",position:"relative",background:"#fff",overflow:"hidden",fontFamily:"'Inter',sans-serif"}}>
      ...
      {tab==="map"&&(
        <div style={{position:"absolute",inset:0}}>
          ...
          <div style={{position:"absolute",right:16,top:"50%",transform:"translateY(-50%)",display:"flex",flexDirection:"column",gap:6,zIndex:10}}>
            {[{l:"+",a:()=>setZoom(z=>Math.min(18,z+1))},{l:"−",a:()=>setZoom(z=>Math.max(8,z-1))},{l:"⌖",a:()=>setMapCenter(DEFAULT_CENTER)}].map(({l,a})=>(
          ...
          {selectedStage&&(
            <><div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.3)",zIndex:39}} onClick={()=>setSelectedStage(null)}/><div className="slide-up" style={{position:"absolute",bottom:0,left:0,right:0,background:"#fff",borderRadius:"16px 16px 0 0",zIndex:40,maxHeight:"88vh",overflowY:"auto"}}><div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px"}}><div style={{width:36,height:4,borderRadius:2,background:"#E0E0E0"}}/></div><StageDetailSheet stage={selectedStage} onClose={()=>setSelectedStage(null)}/></div></>
          )}
          {sheet==="stageBuilder"&&(
      ...
      {/* Course builder */}
      {sheet==="courseBuilder"&&(
        <><div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.3)",zIndex:45}} onClick={()=>setSheet(null)}/><div className="slide-up" style={{position:"fixed",bottom:0,left:0,right:0,background:"#fff",borderRadius:"16px 16px 0 0",zIndex:46,maxHeight:"90vh",overflowY:"auto"}}><div style={{display:"flex",justifyContent:"center",padding:"10px 0 4px"}}><div style={{width:36,height:4,borderRadius:2,background:"#E0E0E0"}}/></div><CourseBuilderSheet stages={stages} onClose={()=>setSheet(null)} onSave={c=>{setCourses(prev=>[...prev,c]);setSheet(null);setCoursesFilter("courses");setTab("stages");}}/></div></>
      )}
